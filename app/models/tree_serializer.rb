class TreeSerializer
  ...
  def self.build(object=nil, depth=2)
    ...
    when "Folder"
      sub_folders = object.children.alpha.collect {|sub_folder| TreeSerializer.build(sub_folder, depth - 1)}.compact

      cloud_files = object.cloud_files.alpha.collect {|cloud_file| TreeSerializer.build(cloud_file, depth - 1)}.compact
      tree = object_attr.merge("entry_type" => "grouping", "klass" => object.class.name.underscore, "children" => (sub_folders + cloud_files))
    when "NilClass"
      folders = Folder.alpha.roots.collect {|folder| TreeSerializer.build(folder,depth - 1)}
